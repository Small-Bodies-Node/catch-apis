...
from catch_apis.services.target_name import TargetTypePatterns
...
COMETS = set([
    ...
    'C/2014 S2 (PanSTARRS)',
    # we are not accepting packed cometary designations at this time
    # 'J95A010',
    # 'J94P01b',
    # 'J94P010',
    # 'K48X130',
...
fn = get_pkg_data_filename('asteroid_closest.txt', package='catch_apis.tests')
...
with open(get_pkg_data_filename('CometEls.txt', package='catch_apis.tests'), 'r') as inf:
    for line in inf:
        if line[0] == '#':
            continue
    ...
    with open(fn, 'r') as inf:
        for line in inf:
            if line.startswith('<pre>'):
                ...
                # remove any html tags, whitespace
...
@pytest.mark.remote_data
@pytest.mark.parametrize('target', RESOLVE_NAMES)
def test_via_name_resolution_tool(target):
